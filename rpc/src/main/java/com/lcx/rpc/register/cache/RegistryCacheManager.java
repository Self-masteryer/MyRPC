package com.lcx.rpc.register.cache;

import com.lcx.rpc.common.exception.RegistryException;
import com.lcx.rpc.model.ServiceMetaInfo;
import lombok.extern.slf4j.Slf4j;

import java.util.*;
import java.util.concurrent.*;

/**
 * 注册中心缓存管理器
 */
@Slf4j
public abstract class RegistryCacheManager {

    protected volatile boolean isDestroyed = false;
    // 服务发现缓存：服务键 -> (节点键 -> 服务)
    protected final Map<String, Map<String, ServiceMetaInfo>> cache = new ConcurrentHashMap<>();
    // 监听器：更新负载均衡器状态
    private final List<CacheUpdateListener<ServiceMetaInfo>> listeners = new CopyOnWriteArrayList<>();
    // 定期全量补偿线程池
    private final ScheduledExecutorService scheduler = Executors.newSingleThreadScheduledExecutor();

    /**
     * 初始化定时补偿任务
     */
    protected void initScheduler(int intervalMinutes) {
        scheduler.scheduleAtFixedRate(
                this::compensationAllCache,
                intervalMinutes,
                intervalMinutes,
                TimeUnit.MINUTES
        );
    }

    /**
     * 加载服务数据（抽象方法）
     *
     * @param serviceKey 服务键
     * @return Map：节点键 -> 服务
     * @throws Exception 异常
     */
    protected abstract Map<String, ServiceMetaInfo> loadServiceData(String serviceKey) throws Exception;

    /**
     * 注册监听器（抽象方法）
     *
     * @param serviceKey 服务键
     */
    protected abstract void registerWatch(String serviceKey);

    /**
     * 处理事件（抽象方法）
     * @param serviceKey 服务键
     * @param event 事件
     */
    protected abstract void handleEvent(String serviceKey, Object event);

    /**
     * 获取服务列表
     */
    public List<ServiceMetaInfo> getServices(String serviceKey) {
        if (!cache.containsKey(serviceKey)) {
            loadAndWatch(serviceKey);
        }
        return Optional.ofNullable(cache.get(serviceKey))
                .map(Map::values)
                .orElse(Collections.emptyList()).parallelStream().toList();
    }

    /**
     * 加载并监听服务
     */
    public void loadAndWatch(String serviceKey) {
        synchronized (cache) {
            if (cache.containsKey(serviceKey)) return;

            try {
                Map<String, ServiceMetaInfo> newData = loadServiceData(serviceKey);
                cache.put(serviceKey, new ConcurrentHashMap<>(newData));
                registerWatch(serviceKey);
                notifyListeners(serviceKey);
            } catch (Exception e) {
                throw new RegistryException("服务加载失败: " + serviceKey, e);
            }
        }
    }

    /**
     * 全量补偿任务
     */
    private void compensationAllCache() {
        if (isDestroyed) return;
        cache.keySet().forEach(serviceKey -> {
            try {
                Map<String, ServiceMetaInfo> newData = loadServiceData(serviceKey);
                cache.put(serviceKey, new ConcurrentHashMap<>(newData));
                log.info("缓存补偿完成: {} nodes={}", serviceKey, newData.size());
            } catch (Exception e) {
                log.error("补偿失败: {}", serviceKey, e);
            }
        });
    }

    /**
     * 通知监听器
     */
    protected void notifyListeners(String serviceKey) {
        listeners.forEach(listener ->
                listener.onUpdate(serviceKey, cache.get(serviceKey).values())
        );
    }

    /**
     * 添加监听器
     */
    public void addListener(CacheUpdateListener<ServiceMetaInfo> listener) {
        listeners.add(listener);
    }

    /**
     * 销毁
     */
    public void destroy() {
        isDestroyed = true;
        scheduler.shutdownNow();
        cache.clear();
    }
}